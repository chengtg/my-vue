...
<!--      <el-form-item label="租金接受范围" prop="rentAcceptance">
      ...
      <el-form-item label="预审状态" prop="preAuditStatus">
        <el-select v-model="dataForm.preAuditStatus" :disabled="optState!=4"  placeholder="选择审核状态"  >
          <el-option
      ...
      <template  v-if="optState!=4">
          <el-form-item label="预审审核人" prop="preAuditorName">
            <el-input v-model="dataForm.preAuditorName"  :disabled="optState!=4" placeholder="预审审核人"></el-input>
          </el-form-item>

          <el-form-item label="预审时间" prop="preAuditTime">
            <el-input v-model="dataForm.preAuditTime" :disabled="optState!=4" placeholder="预审审核时间"></el-input>
          </el-form-item>
      </template>
      <el-form-item label="预审信息" prop="preAuditInfo">
        <el-input v-model="dataForm.preAuditInfo" type="textarea" :disabled="optState!=4"  placeholder="预审审核信息"></el-input>
      </el-form-item>
    </el-form>
  ...
  export default {
    ...
    methods: {
      init(id, optState, pageTitle) {
        ...
        this.isDisabled = !!((optState == '2' || optState == '4'));// 0新增、1修改、2查看、3删除 4、审核
        this.fileList = [];
        this.visible = true;
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
              ...
              if (data && data.code === 0) {
                  ...
                  // 默认审核成功
                  // this.dataForm.preAuditStatus = '1';
                // 文件分类与格式化
      ...
      dataFormSubmit() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/bunk/tmerchantsapply/prestate`),
              method: 'post',
              data: params
            }).then(({data}) => {
              if (data && data.code === 0) {
