<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="140px">
      <el-form-item label="项目名称" prop="projectName">
        <el-input v-model="dataForm.projectName" :disabled="isDisabled" placeholder="项目名称"></el-input>
      </el-form-item>
  ...
  <!--    <el-form-item label="租金接受范围" prop="rentAcceptance">
      ...
      <el-form-item label="复审状态" prop="secondAuditStatus">
        <el-select v-model="dataForm.secondAuditStatus" :disabled="optState!=4" placeholder="选择审核状态"  >
          <el-option
            v-for="item in AUDIT_TYPE_Options"
            :key="item.value"
            :label="item.label"
      ...
      <template  v-if="optState!=4">
        <el-form-item label="复审审核人" prop="secondAuditorName">
          <el-input v-model="dataForm.secondAuditorName"  :disabled="optState!=4" placeholder="复审审核人"></el-input>
        </el-form-item>
          <el-form-item label="复审时间" prop="secondAuditTime">
            <el-input v-model="dataForm.secondAuditTime"  :disabled="optState!=4" placeholder="复审审核时间"></el-input>
          </el-form-item>
      </template>
      <el-form-item label="复审信息" prop="secondAuditInfo">
        <el-input v-model="dataForm.secondAuditInfo" type="textarea" :disabled="optState!=4"  placeholder="复审审核信息"></el-input>
      </el-form-item>
    </el-form>
  ...
  export default {
    data() {
      return {
        ...
        dataRule: {
          preAuditInfo:[
            { max: 400, message: '长度在 400 个字符之内', trigger: 'blur' }
          ]
        }
      };
    },
    ...
    methods: {
      init(id, optState, pageTitle) {
        this.pageTitle = pageTitle;
        this.dataForm.id = id || 0;

        this.optState = optState;
        this.isDisabled = !!((optState == '2' || optState == '4'));// 0新增、1修改、2查看、3删除 4
        this.visible = true;
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
              ...
              if (data && data.code === 0) {
                  ...
                  // 默认审核成功
                  // this.dataForm.secondAuditStatus = '1';
                  // 文件分类与格式化
      ...
      dataFormSubmit() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/bunk/tmerchantsapply/secondstate`),
              method: 'post',
              data: params
            }).then(({data}) => {
              if (data && data.code === 0) {
