    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="140px">
      <el-form-item size="mini" label="存储类型">
        <el-radio-group v-model="dataForm.type">
         <!-- <el-radio :label="5">应用服务器存储</el-radio>-->
          <el-radio :label="4">文件服务器</el-radio>
      ...
      <template v-else-if="dataForm.type === 4">
        <el-form-item label="存储ip和端口">
          <el-input v-model="dataForm.trackerList" placeholder="存储ip和端口"></el-input>
        </el-form-item>
        <el-form-item label="读取超时(毫秒)">
          <el-input v-model="dataForm.soTimeout" placeholder="读取超时(秒)"></el-input>
        </el-form-item>
        <el-form-item label="连接超时(毫秒)">
          <el-input v-model="dataForm.connectTimeout" placeholder="连接超时(秒)"></el-input>
        </el-form-item>
        <el-form-item label="访问ip和端口">
          <el-input v-model="dataForm.accessUrl" placeholder="访问ip和端口" ></el-input>
        </el-form-item>
      </template>
      <!--应用服务器存储-->
      <template v-else-if="dataForm.type === 5">
        <el-form-item label="后台管理存储路径">
          <el-input v-model="dataForm.sysUploadPath" placeholder="后台管理存储路径"></el-input>
        </el-form-item>
        <el-form-item label="商户资料存储路径">
          <el-input v-model="dataForm.customFileLocalPath" placeholder="商户资料存储路径"></el-input>
        </el-form-item>
        <el-form-item label="外网访问路径">
          <el-input v-model="dataForm.wwwUrl" placeholder="外网访问路径" ></el-input>
        </el-form-item>
      </template>
    </el-form>
